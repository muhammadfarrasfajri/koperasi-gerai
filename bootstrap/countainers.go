...
func InitContainer(userAuth *auth.Client) *Container {
	...
	adminDeviceTokenRepo := repository.NewAdminDeviceTokenRepository(database.DB)
	...
	messagingClient, _ := config.FirebaseAppUser.Messaging(context.Background())
	notificationRepo := repository.NewFCMNotificationRepository(messagingClient)
	notificationService := services.NewNotificationService(
		notificationRepo,
		adminDeviceTokenRepo,
	)
	eventBus := services.NewInMemoryEventBus(notificationService)

	userAuthService := services.NewUserAuthService(userAuthRepo, userRepo, userRefRepo, userAuth, jwtManager, eventBus)
